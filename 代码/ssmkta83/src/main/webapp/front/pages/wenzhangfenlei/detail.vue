...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','wenzhangfenlei');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','wenzhangfenlei');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('wenzhangfenlei', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusswenzhangfenlei/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('wenzhangfenlei', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('wenzhangfenlei', this.detail);
				this.$utils.msg('审核成功');
