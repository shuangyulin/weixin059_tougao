	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusswenzhangzhanshi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`discusswenzhangzhanshi`, this.ruleForm);
				}else{
					await this.$api.add(`discusswenzhangzhanshi`, this.ruleForm);
				}
				uni.setStorageSync('discusswenzhangzhanshiCleanType',true);
