	...
	<view class="content">
		<form class="app-update-pv">
			...
			<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">文章分类</view>
				<picker @change="wenzhangfenleiChange" :value="wenzhangfenleiIndex" :range="wenzhangfenleiOptions">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.wenzhangfenlei?ruleForm.wenzhangfenlei:"请选择文章分类"}}</view>
				</picker>
			</view>
			<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">编辑时间</view>
				<picker mode="date" :value="ruleForm.bianjishijian" @change="bianjishijianChange">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.bianjishijian?ruleForm.bianjishijian:"请选择编辑时间"}}</view>
				</picker>
			</view>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
    		        this.ruleForm.bianjishijian = this.$utils.getCurDate();
			...
			// 下拉框
			res = await this.$api.option(`wenzhangfenlei`,`wenzhangfenlei`,{});
			this.wenzhangfenleiOptions = res.data;
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`wenzhangzhanshi`, this.ruleForm.id);
		...
		methods: {
			...
			bianjishijianChange(e) {
				this.ruleForm.bianjishijian = e.target.value;
				this.$forceUpdate();
			},
			...
			// 下拉变化
			wenzhangfenleiChange(e) {
				this.wenzhangfenleiIndex = e.target.value
				this.ruleForm.wenzhangfenlei = this.wenzhangfenleiOptions[this.wenzhangfenleiIndex]
			},
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`wenzhangzhanshi`, this.ruleForm);
				}else{
					await this.$api.add(`wenzhangzhanshi`, this.ruleForm);
				}
