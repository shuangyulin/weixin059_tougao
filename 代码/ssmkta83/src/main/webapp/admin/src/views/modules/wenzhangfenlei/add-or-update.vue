<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="文章分类" prop="wenzhangfenlei">
          <el-input v-model="ruleForm.wenzhangfenlei" 
              placeholder="文章分类" clearable  :readonly="ro.wenzhangfenlei"></el-input>
        </el-form-item>
        <div v-else>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wenzhangfenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wenzhangfenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
