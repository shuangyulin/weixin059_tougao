...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          bianjizhanghao: [
                { required: true, message: '编辑账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          bianjixingming: [
                { required: true, message: '编辑姓名不能为空', trigger: 'blur' },
          ],
          nianling: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `bianji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.bianjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
